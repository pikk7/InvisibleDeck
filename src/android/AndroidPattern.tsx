...
export default function AndroidPattern({
  ...
  const [hud, setHud] = React.useState({ quadrant: "-", count: 0, rank: "-" });
  ...
  return (
    <div className="screen pattern">
      <div className="pt-title">Rajzold meg a mintát</div>
      <canvas id="pad" ref={ref} width={360} height={480} />
      <div className="pt-footer">Mintás feloldás (4×4)</div>

      {hudVisible && (
        <div className="hud">
          <div className="hud-row">
            <span className="hud-label">Kvadráns:</span>{" "}
            <span className="hud-val">{hud.quadrant}</span>
          </div>
          <div className="hud-row">
            <span className="hud-label">Pontok:</span>{" "}
