...
function mapSuitDigit(d: number) {
  // 1→♥, 2→♦, 3→♣, 4→♠; 5→♥, 6→♦, 7→♣, 8→♠, 9→♥, 0→♠
  const map: Record<number, number> = {1:0,2:1,3:2,4:3,5:0,6:1,7:2,8:3,9:0,0:3};
  return map[d] ?? 0;
}
...
export default function IOSPasscode({ onCard }: { onCard:(c:Card)=>void }) {
  ...
  const add = (d:number)=>{ if(pin.length<6) setPin(p=>[...p,d]); };
  ...
  React.useEffect(()=>{
    if(pin.length===6){
      ...
      const card: Card = { suit: ['hearts','diamonds','clubs','spades'][suitIdx] as Card['suit'], rank: ranks[val-1] };
  ...
  return (
    <div className="screen passcode">
      <div className="pc-title">Írja be a jelkódot</div>
      <div className="dots">{[0,1,2,3,4,5].map(i=><span key={i} className={`dot ${i<pin.length?'filled':''}`}/>)}</div>
      <div className="keypad" role="group" aria-label="Számbillentyűzet">
        {[1,2,3,4,5,6,7,8,9].map(d=><button key={d} onClick={()=>add(d)}>{d}</button>)}
        <div className="kp-spacer" />
        <button onClick={()=>add(0)}>0</button>
        <button className="del" onClick={del}>⌫</button>
      </div>
      <div className="pc-footer">Sürgősségi • Visszahívás</div>
    </div>
  );
}
