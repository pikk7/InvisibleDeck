...
function reducer(state: Settings, action: Action): Settings {
  switch (action.type) {
    ...
    case 'SET_HUD':          return { ...state, hud: action.payload };
    case 'SET_FORCE_MODE':   return { ...state, forceMode: action.payload };
    case 'SET_DEBUG_FACEID': return { ...state, debugFaceID: action.payload };
    case 'RESET_DEFAULTS':   return { ...defaultSettings };
    default:                 return state;
  }
}

const SettingsContext = createContext<{ settings: Settings; dispatch: React.Dispatch<Action>; } | null>(null);
